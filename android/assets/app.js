(() => {
  ...
  let translationRequest=0,translationTimer;
  let handwritingRequest=0,photoRequest=0,photoPending=false,photoRows=[];
  ...
  function change(fn) {const next=E.clone(state);const result=fn(next);E.unlockStickers(next);persist(next);state=next;return result;}
  ...
  const protectedScreen=s=>['parent','settings','report'].includes(s);
  ...
  function render() {
    ...
    ({home,sets,detail,train,cards,wallet,parent,editor,report,settings,result,importPreview,album,photoReview}[screen]||home)();
  }
  function welcome() {
  ...
  function home() {
    ...
    shell(`<div class="row between top"><span class="eyebrow"><span class="brand-dot"></span>Маленькие открытия</span>${button('⚙','parent','iconbtn','aria-label="Родителям"')}</div><div class="hero">${brand()}<div class="eyebrow">Твоя английская история</div><h1>Английские слова<span>для ${esc(state.profile.genitive)}</span></h1><p class="muted">По одному слову к новым открытиям</p></div><button class="wallet-link" data-action="wallet"><span class="gift">🎁</span><div class="grow"><strong class="smallword">Копилка времени</strong><div class="caption">Твои минуты на Roblox</div></div><div class="balance"><strong>${state.wallet.balance}</strong><small>МИНУТ ›</small></div></button><div class="row between section-title"><h3>Твоё занятие</h3>${d?.due?`<span class="badge">${esc(dateLabel(d.due))}</span>`:''}</div>${d?`<section class="card"><div class="study-picker"><span id="study-label">Что будем практиковать?</span><button id="study-target" class="scope-trigger" data-action="choose-scope" data-id="${esc(selectedDeck)}" aria-haspopup="dialog" aria-expanded="false" aria-labelledby="study-label study-value"><span id="study-value">${esc(d.title)}</span><span class="scope-chevron" aria-hidden="true">⌄</span></button></div><div class="eyebrow mt">${all?'Смешанная тренировка':'Только выбранный набор'}</div><h2 class="mt-sm">${esc(d.title)}</h2><p class="note">${wordCount(p.total)} · 10 заданий · в своём темпе</p><div class="meter-label"><span>Помню перевод</span><strong>${p.translation} / ${p.total}</strong></div>${meter(p.translation,p.total)}<div class="meter-label"><span>Пишу самостоятельно</span><strong>${p.spelling} / ${p.total}</strong></div>${meter(p.spelling,p.total)}<div class="mt">${button('Начать занятие <span aria-hidden="true">→</span>','start-auto')}</div>${button('Сначала посмотреть карточки','open-cards','textbtn')}</section>`:`<section class="card empty"><span class="symbol">💌</span><h2>Ждём первые слова</h2><p class="note">Открой набор, который прислал папа, или попроси взрослого создать его здесь.</p><div class="mt">${button('Открыть файл со словами','pick-file')}</div></section>`}<div class="row between section-title"><h3>Потренируемся?</h3></div><div class="levels">${E.LABELS.map((l,i)=>`<button class="level" data-action="choose-band" data-band="${i}" ${!d?'disabled':''}><span class="symbol">${symbols[i]}</span><strong>${l}</strong><small>+${state.settings.rewards[i]} мин / 10 верных</small></button>`).join('')}</div>${d?`<div class="mt-sm">${button('Повторить трудные слова','start-hard','secondary')}</div>`:''}<div class="mt">${button('🏆 Мой альбом · '+Object.keys(state.stickers||{}).length+' / '+E.STICKERS.length,'album','secondary tint')}</div><p class="quote">С любовью для ${esc(state.profile.genitive)} ♡</p>`);
  }
  function album(){
    const progress=E.stickerProgress(state),earned=state.stickers||{};
    shell(`${top('Мой альбом')}<div class="hero"><div class="word">🏆</div><h1>Маленькие открытия</h1><p class="muted">${Object.keys(earned).length} из ${E.STICKERS.length} наклеек в коллекции</p></div><p class="note">Наклейки остаются навсегда. Занимайся в своём темпе — дни не обязательно должны идти подряд. Слово считается изученным после верных ответов в разные дни.</p><div class="sticker-grid mt">${E.STICKERS.map(x=>{const got=Object.hasOwn(earned,x.id),n=Math.min(progress[x.kind],x.goal);return `<article class="card sticker ${got?'unlocked':'locked'}"><div class="sticker-art" aria-hidden="true">${got?x.icon:'🔒'}</div><h3>${esc(x.title)}</h3><p class="note">${esc(x.rule)}</p>${got?`<span class="chip done">В коллекции ✓</span><p class="caption">${stamp(earned[x.id])}</p>`:`${meter(n,x.goal)}<p class="caption">${n} / ${x.goal}</p>`}</article>`;}).join('')}</div>`);
  }
  function photoReview(){
    shell(`${top('Слова с фотографии','photo-cancel')}<h1>Проверим список</h1><p class="note">Исправьте распознанные английские слова, проверьте предложенные переводы и снимите галочки с лишних строк. Фото не добавляет набор автоматически.</p><div class="form"><label for="photo-title">Название набора</label><input id="photo-title" maxlength="100" value="Набор с фотографии"></div><p class="note" id="translation-status" role="status">Перевод — подсказка: выберите значение, которое подходит к уроку.</p>${button("Предложить переводы","photo-translate","secondary tint")}<div class="stack mt">${photoRows.map((word,i)=>`<section class="card form photo-row"><label><input type="checkbox" id="photo-use-${i}" checked> Включить в набор</label><label for="photo-en-${i}">Английское слово</label><input id="photo-en-${i}" maxlength="60" value="${esc(word)}" lang="en" autocomplete="off"><label for="photo-ru-${i}">Перевод</label><input id="photo-ru-${i}" maxlength="800" placeholder="Например, медленный"></section>`).join('')}</div><div id="form-error" class="error" role="alert"></div><div class="mt">${button('Продолжить →','photo-accept')}</div>${button('Отменить','photo-cancel','secondary')}`,false);
  }
  function startTranslation(){
    if(screen!=='photoReview'||!A?.translatePhoto)return;
    const words=photoRows.map((_,i)=>$('#photo-en-'+i).value.trim());
    if(words.some(w=>!/^[a-zA-Z '’-]{1,60}$/.test(w))){$('#translation-status').textContent='Исправьте английские слова перед переводом.';return;}
    const request=String(++translationRequest);clearTimeout(translationTimer);
    $('#translation-status').textContent='Готовим переводы… При первом запуске загружается языковая модель (около 30 МБ). Затем перевод работает без интернета.';
    A.translatePhoto(request,JSON.stringify(words));
    translationTimer=setTimeout(()=>{if(screen==='photoReview'&&String(translationRequest)===request){translationRequest++;$('#translation-status').textContent='Переводчик пока не ответил. Проверьте интернет и нажмите «Предложить переводы» ещё раз. Можно заполнить поля вручную.';}},90000);
  }
  function onTranslation(request,index,source,value,done){
    if(screen!=='photoReview'||request!==String(translationRequest))return;
    if(done){clearTimeout(translationTimer);$('#translation-status').textContent=value||'Переводы предложены. Проверьте их и исправьте, если нужно.';return;}
    if(!Number.isInteger(index)||index<0||index>=photoRows.length||typeof value!=='string')return;
    const en=$('#photo-en-'+index),ru=$('#photo-ru-'+index);
    if(en.value.trim()!==source||ru.value.trim()||ru.dataset.edited)return;
    ru.value=E.capital(value.slice(0,800));ru.dataset.suggested='1';
  }
  function receivePhoto(request,text,error){
    if(!photoPending||String(photoRequest)!==request)return;photoPending=false;
    if(error){toast(error);return;}if(!text)return;
    try{const rows=E.photoWords(text);if(!rows.length)throw Error('Английские слова не найдены. Попробуйте более чёткое фото списка.');
      photoRows=rows;openScreen('photoReview');startTranslation();
    }catch(e){err(e);}
  }
  ...
  function deckMenu(id) {
    ...
    modal(d.title,`<div class="stack mt">${button('Редактировать','menu-edit','secondary')}${button('Отправить набор','menu-share','secondary')}${button(d.archived?'Вернуть из архива':'В архив','menu-archive','secondary')}${button('Удалить навсегда','menu-delete','secondary danger')}</div>`,null,null,'Закрыть');
  }
  function detail() {
  ...
  async function action(a,el) {
    switch(a){
      ...
      case 'album':change(()=>{});openScreen('album');return;
      case 'photo-new':{modal('Набор по фотографии','<p>Выберите фото из галереи или сделайте снимок списка. Лучше всего распознаётся печатный английский текст. Приложение предложит русские переводы — проверьте и поправьте их. Для первого перевода понадобится интернет: загрузится бесплатная языковая модель (около 30 МБ).</p><p>Фото обрабатывается на телефоне.</p>','Выбрать фото',()=>{closeModal();if(!A?.scanPhoto){toast('Фото можно распознать в Android-приложении.');return;}photoPending=true;A.scanPhoto(String(++photoRequest));});}return;
      case 'photo-translate':startTranslation();return;
      case 'photo-cancel':translationRequest++;clearTimeout(translationTimer);photoRows=[];photoPending=false;openScreen('sets');return;
      case 'photo-accept':{const title=$('#photo-title').value;const words=photoRows.flatMap((_,i)=>$('#photo-use-'+i).checked?[{id:E.uid(),en:E.capital($('#photo-en-'+i).value),ru:$('#photo-ru-'+i).value.split(';').map(E.capital).filter(Boolean)}]:[]);const d=E.cleanDeck({id:E.uid(),title,words});{editId='';draft={title:d.title,due:'',lines:d.words.map(w=>w.en+' — '+w.ru.join('; ')).join('\n')};photoRows=[];openScreen('editor');}return;}
      ...
      case 'menu-delete':closeModal();await action('delete-deck');return;
      ...
      case 'handwriting':if(answered||!$('#answer'))return;if(A?.openHandwriting){A.openHandwriting(String(++handwritingRequest),q.mode==='gap',q.mode==='write'?q.word.ru.join('; '):q.mode==='gap'?q.mask:'',q.mode==='listen');}else{toast('Самостоятельное рукописное поле доступно в Android-приложении.');}return;
      ...
      case 'new-deck':{editId='';draft=null;openScreen('editor');}return;
      case 'edit-deck':{editId=deck().id;draft=null;openScreen('editor');}return;
      ...
      case 'save-deck':{draft={title:$('#deck-title').value,due:$('#deck-due').value,lines:$('#deck-lines').value};{const old=state.decks.find(d=>d.id===editId);const d=E.cleanDeck({id:editId||E.uid(),title:draft.title,due:draft.due,words:E.parseLines(draft.lines,old)});modal('Сохранить набор?',`<p><strong>${esc(d.title)}</strong> · ${wordCount(d.words.length)}</p><p>${esc(d.words.map(w=>w.en+' — '+w.ru.join('; ')).join('\n')).replace(/\n/g,'<br>')}</p>`,'Сохранить',()=>{change(s=>E.install(s,d));selectedDeck=d.id;draft=null;closeModal();openScreen('detail');});}return;}
      case 'export-deck':exportFile(E.exportDeck(deck()),'slova-'+deck().id+'.maywords','deck');return;
      case 'archive':{change(s=>{const d=s.decks.find(d=>d.id===selectedDeck);d.archived=!d.archived;if(d.archived&&s.active===d.id)s.active=E.preferred(s)?.id||'';});openScreen('sets');}return;
      case 'delete-deck':{const d=deck();modal('Удалить набор?',`<p>«${esc(d.title)}» и его учебный прогресс будут удалены. Заработанные минуты сохранятся.</p>`,'Удалить набор',()=>{change(s=>{s.decks=s.decks.filter(x=>x.id!==d.id);d.words.forEach(w=>delete s.progress[E.key(d,w)]);if(s.active===d.id)s.active=E.preferred(s)?.id||'';});closeModal();openScreen('sets');});}return;
  ...
  root.addEventListener('input',e=>{if(e.target.id?.startsWith('photo-ru-'))e.target.dataset.edited='1';if(e.target.id?.startsWith('photo-en-')){const ru=$('#photo-ru-'+e.target.id.slice(9));if(ru?.dataset.suggested&&!ru.dataset.edited){ru.value='';delete ru.dataset.suggested;}}});
